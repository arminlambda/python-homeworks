...
def mozen_skok(x, y, ovira):
    y1, x0, x1 = ovira
    mozne_tocke = [(x+1,y-2), (x+1,y-1), (x+2,y-1),
                   (x+2,y), (x+1,y+2), (x+1,y+1), (x+2, y+1)]
    ovira = [(x0,y1)for x0 in range(x0, x1 + 1)]
    for i in ovira:
        if i in mozne_tocke:
            return True
...
def mozne_ovire(x, y):
    seznam_ovir = []
    for i in ovire:
        if mozen_skok(x, y, i) == True:
            seznam_ovir.append()
    return seznam_ovir

def mozne_ovire(x,y):
    return [i for i in ovire if mozen_skok(x,y,i) == True]
...
def obstaja_pot(ovira):
    return True if ovira[2] == 16 else any(obstaja_pot(i) for i in mozne_ovire(ovira[2], ovira[0]))
