...
def vrednost_povezave(povezava, zemljevid):
    vsota = 0
    for x in zemljevid[povezava]:
        vsota += tockovanje_vescin[x]
    return vsota

def najboljsa_povezava(zemljevid):
    najvec = 0
    for x in zemljevid.keys():
        trenutna = vrednost_povezave(x,zemljevid)
        if trenutna > najvec:
            najvec = trenutna
            terka = x
    return terka
...
def vrednost_poti(pot, zemljevid):
    ...
    for i in range(len(pot)-1):
        kljuc = (pot[i],pot[i+1])
        vsota += vrednost_povezave(kljuc,zemljevid)
    return vsota

def  najbolj_uporabna(pot, zemljevid):
    slovar = {}
    for i in range(len(pot)-1): #ABRCVA
        kljuc = (pot[i], pot[i+1])
        for veščina in zemljevid[kljuc]:
            if veščina in slovar:
                slovar[veščina] += 1
            else:
                slovar[veščina] = 1
    if len(slovar) == 0:
